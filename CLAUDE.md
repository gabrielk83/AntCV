...
2. **CLAIM A LANE (NIGHT SHIFT).** Before editing anything that consumes a version number (any `pwa/` asset needing a cache-bust) or that a parallel session might touch, run `node scripts/shift.mjs claim --task "<what>"`. It reserves a version-number RANGE in `docs/qa/NIGHT_SHIFT.md` (computed from the true git high-water mark, so a regressed `TARGET_VERSION` can't hand out used numbers) and prints a `git worktree add` line. **Work in that worktree**, use version numbers only inside your range, `node scripts/shift.mjs release` when done. This is what stops the cross-session version collisions + shared-tree WIP bleed (the 245/246 regression class). `shift.mjs status` reads origin, so it is correct even when your tree is dirty. Docs-only edits may skip the claim but should still SYNC FIRST. **One claim per clone/worktree:** the session id lives in that checkout's gitdir (`.git/shift-session-id` or `.git/worktrees/<name>/shift-session-id`), so `claim` REFUSES when the checkout you run it from already holds a live row (SHIFT-SHARED-ID-001, 2026-09-06 — a second claim from the shared main clone used to overwrite the first session's row, and the first session's `release` then deleted the second's). If the main clone is already claimed, `git worktree add ../AntCV-<name> -b <name>` first and run `claim` from INSIDE that worktree; run `beat`/`release` from the checkout that made the claim (its id file), or copy that id file into the worktree's gitdir. If a push is rejected because origin moved, the tool rebuilds the ledger block from origin and rebases; if it prints `NOT PUSHED`, origin does not have your change — fix it before continuing.
