...
| AUTO-PAGEBREAK-BLOCK-001 | Automated page breaks: **always** show the salmon splitter when content exceeds one A4 page in preview; sliding is **block-level** — a whole sub-subsection moves to the next page (never partial, never the whole parent subsection). | 🟡 1.50.265 — CV + CL coverage incl. table-row split | Owner spec 2026-06-06. `antcv-auto-pagebreak-block-001.js` covers: (1) whole-section move at any `data-sid` boundary; (2) plain-list intra-split for sections with header + N body blocks (EXPERIENCE roles, OUTCOMES bullets, CERTIFICATIONS/EDUCATION/PUBLICATIONS/ADDITIONAL lists, CL Foundation hands_on↔professionally, CL HWIC intro/bullets/closing); (3) group-aware intra-split for labeled-list sections (mix of group sub-headers + items via `data-antcv-row-path` — REGULATORY, TOOLS-with-groups, including hidden labels via display:none); (4) table-row intra-split for sections containing a table (CORE COMPETENCIES, CL "What I bring") — thead cloned onto each continuation page; (5) single-text/headers protected: any section that's just header + 1 unsplittable child falls back to whole-section move (no internal split). 1.50.265 also adds "oversized first child" fall-through so a single-text section taller than one page no longer pins the next section to its oversized row. Salmon splitter rendered between page-rows, hidden in print. Idempotent (undo previous splits before re-splitting, re-stitching tables and sections by data-sid match), debounced MutationObserver, 6-split safety cap. Synthetic probes verified each case independently. **Follow-up:** (a) "(continued)" header on continuation pages, (b) carry sidebar photo / contact strip on page 2+ as a slim header, (c) reconcile with manual page=2,3,4 markers + PB-001..006 + EXPORT-PAGE2-001. |
