...
| AUTO-PAGEBREAK-BLOCK-001 | Automated page breaks: **always** show the salmon splitter when content exceeds one A4 page in preview; sliding is **block-level** — a whole sub-subsection moves to the next page (never partial, never the whole parent subsection). | 🟡 1.50.264 — group-aware intra-split | Owner spec 2026-06-06. `antcv-auto-pagebreak-block-001.js` splits at top-level section boundary (`data-sid`) AND intra-section between any section's child blocks. Now covers: EXPERIENCE roles, OUTCOMES bullets, REGULATORY by GROUP (label+items glued), TOOLS rows, CERTIFICATIONS list, EDUCATION list, PUBLICATIONS list, ADDITIONAL INFO rows. Group-aware logic (1.50.264): when a section mixes group sub-headers + items (identified by `data-antcv-row-path`), valid split points are restricted to sub-header indices so the label always travels with its body. Sections with only one splittable child (CORE COMPETENCIES table, work_style paragraph) auto-fall-back to whole-section move. Salmon splitter rendered between page-rows, hidden in print. Idempotent, debounced MutationObserver, 6-split safety cap. Synthetic probes verified: EXPERIENCE 6 roles → 3+3; REGULATORY 8 items → 6+2; REGULATORY 4 groups × 2 items → 2 groups + 2 groups (no orphaned label). **Follow-up:** (a) table-row intra-split for CORE COMPETENCIES, (b) "(continued)" header on continuation pages, (c) carry sidebar photo / contact strip on page 2+ as a slim header, (d) reconcile with manual page=2,3,4 markers + PB-001..006 + EXPORT-PAGE2-001. |
