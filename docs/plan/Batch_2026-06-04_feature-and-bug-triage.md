...
| ID | Statement | Layer | Verdict |
|----|-----------|-------|---------|
| MERGE-DUP-001 | Merge the two **Writing style** selects (legacy "WRITING STYLE" + new "Writing style — was X") into one, keep the new copy's id→"was X" mapping & description | APP.JS | DOM lives in the app.js Set panel. |
| MERGE-DUP-002 | Merge duplicate **Preferred tone / "Tone chips"** sections; the first active tone chip is a broken run-on string ("calm and direct… explicit.") that should be split/normalised; move the 6 tone chips into the original section | APP.JS | |
| MERGE-DUP-003 | Merge "save tones" into "save customs" ("+ Save current as new slot" duplicate vs the Custom-slot save/load/clear) | APP.JS | |
