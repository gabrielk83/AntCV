> **AUTHORITATIVE open/closed + copy-paste next-session prompt: `docs/qa/PROJECT_ISSUES_OPEN_CLOSED_2026-06-30.md`** (PWA **1.50.999** · docx-worker **1.14.102** · access-relay **1.3.2** · suite 529/529). Closed last 3 batches = 41; OPEN now = 18. Just shipped (owner-verify on a real export): CV admin template (1.50.995), contact-line bridge no-shrink (wk 1.14.101), Mgmt→Management + headline-off vertical cue (1.50.996), CL signature-clip fix (wk 1.14.102). TOP OPEN now includes rich_block inline-edit PERSISTENCE (data-loss class — needs live repro) + bring_intro + lead-in colons. _(prior snapshot:)_ **`docs/qa/PROJECT_ISSUES_OPEN_CLOSED_2026-06-29.md`** + per-item plans in `docs/qa/NEXT_SESSION_2026-06-29.md` (PWA **1.50.968** + docx-worker **1.14.96** + access-relay **1.3.2**; suite 521/521). Read those FIRST. CRITICAL data-loss FIXED (relay 1.3.2 + client guard 1.50.957). Top OPEN = (1) kernel role bullets/results + Students-Council dup (owner rules: hide bullet not result; persistence), (2) candidate-header 3-col-grid placement, (3) CL sign-off/rich_block-compress/slogan-panel, (4) quick-gen page convergence. Older: `NEXT_SESSION_2026-06-28.md`.
>
