> **STALE-TEST HYGIENE 2026-07-15 — GROUP-EMPTY-HIDE-001 mirror-lock strings refreshed (test-only; commit 5ccc51d, branch `claude/dreamy-kalam-1d5a03`, pushed; no version bump, no app change).** `pwa/test/unit/group-empty-hide.test.mjs` had 2 RED "mirror-lock" string tests on clean origin/main — NOT introduced by any recent work. The behaviour + parity half of the suite (which brace-extracts the REAL `__grpHasChild` from app.src.js + the worker and runs the 9-case fixture table) stayed GREEN, so the empty-group hide logic is intact; only the literal `.includes()` expectation strings were stale. **Cause:** a cloud session refactored the grp-branch guard to skip user-created empty groups — the drop now reads `if(!r.grpKeep&&!__gc(n))return null` (minified app.js) / `if (!row.grpKeep && !__grpHasChild(i)) return …` (app.src.js + docx-worker), so the old `if(!__gc(n))return null` / `if (!__grpHasChild(i)) return …` substrings no longer matched. Same class as the earlier `<th>`-guard mirror-lock in `table-header-center.test.mjs`. **Fix:** updated ONLY the 3 expectation strings to the current grpKeep-aware form (GROUP-EMPTY-HIDE-001 sentinel + `__gc`/`x&&"object"==typeof x&&x.grp)break` scan assertions unchanged). Verified the current guard still hides childless groups (grpKeep = an explicitly user-made group stays visible while rows are added). Suite `node scripts/run-tests.mjs pwa` → 1313/1313, 0 fail. Rebased onto latest origin/main (clean, no file overlap; merge sim = 0 conflict markers) before pushing the feature branch. **No OPEN_REGISTER row** — row 52 stays SHIPPED; this is test-tracking-parity only, mirroring the grpKeep refactor.
>
