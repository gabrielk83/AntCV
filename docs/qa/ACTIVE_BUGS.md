> **ROADMAP — ROWFIT feedback granularity + group-level actions (owner-requested 2026-07-22, after ROWFIT-PINK-LATCH-001 shipped `1.51.2800`).** The row-level Fit-it (⇥) / Enhance (✨) feature is DONE and both feedback cues (⏳ + section pink) are storm-proof (module-level latches). Owner requested these follow-ups for a future pass:
> - **ROWFIT-PINK-ROWLEVEL** — pink is currently applied to the WHOLE SECTION (the preview `Ce` `R(...)` wrapper wraps the entire section, `data-sid=e.id`). Owner wants per-ROW pink. Needs the rich_block row render (inside `Ce`, `case "rich_block"` ~app.src.js 6484) to wrap the ACTING row in the pink style, keyed off a per-row module latch (mirror `window.__antcvSectionBusy` but `[sectionId+":item:"+i]`; `il`/`ll` already write the per-row `kr["sectionId:item:i"]` key, and the editor already stamps `__rowBusy["e:/c:item:i"]`). More invasive than the section latch — the row render isn't currently wrapped per-row.
> - **ROWFIT-PINK-ROLEGROUP** — pink at ROLE and GROUP level too (experience roles rendered per-role already have a per-role `kr` key; group sub-headings do not). Extend the per-row latch to role ids + group rows.
> - **ROWFIT-GROUP-BUTTONS** — the ✨ Enhance / ⇥ Fit-it buttons are MISSING on GROUP-level rows (group sub-headings, `grp:true`). In `antcv-rich-block-editor.js` the group-row branch (~line 130-149) renders a different control set WITHOUT onEnrich/onCompress buttons; content rows (~line 173-174) have them. Add group-level enhance/fit that acts on the group's child rows (or the group label), routing through a group-scoped payload.
>
