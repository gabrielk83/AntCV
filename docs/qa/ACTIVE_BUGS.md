...
- **RESULTS-DOCX-MISSING-001** — per-role Results are NOT exported to DOCX; they ARE in the PDF (but not for ALL positions). Since the owner's PDF = CloudConvert of the same worker DOCX, the gap is likely a render-branch/condition in `workers/docx-worker/src/index.js renderExperience` (where/whether `role.results` is emitted), and the "not all positions" part overlaps RESULTS-TIGHTENING-STRIP-001.
- **RESULTS-PDF-INK-BLACK-001** — in the exported PDF the per-role Results line renders BLACK instead of greenish; per MAIN-HEADINGS-GREEN-001 the "Results:" label (and/or the line) should be teal `#00746E`. Check the worker's role.results render colour (the experience Results run) — it likely uses a hardcoded/text colour instead of `mainHeadColor`/`mainSubHeadColor`.
- **AI-WATERMARK-EXPORT-LOCATION-001** — the AI watermark sits in the CORRECT section in the CV PREVIEW but the WRONG one in the EXPORT. Design rule: watermark goes in the section whose LAST page has LESS text ([[design-rules-watermark-table]]). The worker's watermark placement disagrees with the preview's; align them.
- **SECTION-TYPE-NORMALIZE-INLINE-001** — normalize `work_style`, `who_i_am`, `why_company`/`why_role` to type `text_inline` (on import in `antcv-data-importer.js` and/or the me() kernel) so the bold inline label ("Work style:", "Who I am:", "Why…:") renders. The owner's `AntCV_sections_corrected_..._outcome_edits.json` stores `work_style`/`profile` as type `text`, so imported data shows no inline label even though the renderer supports it. (Generation already emits `text_inline` post-1.50.497; this covers the IMPORT path.)
