> **HEADER-APP-LINE-001 — HANDED OFF to the CL-heading-spec render lane; my sidecar covers the preview meanwhile (status 2026-07-22).** Owner refinement was: CV has NO application line; the CL heading shows the SPECIALIZATION (not "Application: [role]"); the application line sits UNDER THE SLOGAN. My preview sidecar `antcv-application-line-001.js` (1.51.2520) delivers all three live (CL-only `isCL()` gate; `swapHeadingToSpec()`; line under slogan/contact) — a PREVIEW-LAYER patch. The DURABLE render fix is owned by a PARALLEL lane, CL-APP-SUBTITLE-HEADING-SWAP-001 (claimed 2541-2560), which has since SHIPPED `1.51.2541-cl-heading-spec` + `1.51.2561-cl-spec-center` and is still reworking the header (`2601-header-elem-colors`, `2622-cl-cv-two-tone`). I STOOD DOWN on the render/export change to avoid a direct collision on the same lines (shift discipline). **Remaining, owned by that lane:** (a) confirm the render swap fully replaces "Application: [role]" in BOTH preview render sites (`app.src.js` ~45285 + ~29710 export) and the docx-worker header (EXPORT PARITY still open); (b) retire/disable my `antcv-application-line-001.js` sidecar once the render owns it, so the CL never shows a DUPLICATE application line (heading + under-slogan). Slogan itself: separate — must be a SHARP candidate×role observation, not the spec triad (memory `cl-slogan-content-spec`).
