> **AUTHORITATIVE open/closed + copy-paste AUTONOMOUS next-session prompt: `docs/qa/PROJECT_ISSUES_OPEN_CLOSED_2026-06-30.md`** (PWA **1.51.4** · docx-worker **1.14.105** · access-relay **1.3.2** · suite 529/529). Closed recent batches = 49; OPEN now = 15. **Next session: work ALL remaining OPEN issues AUTONOMOUSLY, START with #1 = rich_block inline-edit PERSISTENCE in the LIVE BROWSER (Chrome MCP), then council laminator + results-dup (hide in NEW docs), empty `[Role title]` positions, A (rule-without-headline + export the vertical cue), then render-gated.** Shipped 2026-07: per-row lead colon (editable + exportable, wk 1.14.103), "Professionally that" casing, bring_intro (1.51.1), signature table-wrap (wk 1.14.104). _(prior snapshot:)_ **`docs/qa/PROJECT_ISSUES_OPEN_CLOSED_2026-06-29.md`** + per-item plans in `docs/qa/NEXT_SESSION_2026-06-29.md` (PWA **1.50.968** + docx-worker **1.14.96** + access-relay **1.3.2**; suite 521/521). Read those FIRST. CRITICAL data-loss FIXED (relay 1.3.2 + client guard 1.50.957). Top OPEN = (1) kernel role bullets/results + Students-Council dup (owner rules: hide bullet not result; persistence), (2) candidate-header 3-col-grid placement, (3) CL sign-off/rich_block-compress/slogan-panel, (4) quick-gen page convergence. Older: `NEXT_SESSION_2026-06-28.md`.
>
