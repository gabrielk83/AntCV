> **AUTHORITATIVE open/closed + copy-paste next-session prompt: `docs/qa/PROJECT_ISSUES_OPEN_CLOSED_2026-06-30.md`** (PWA **1.50.995** · docx-worker **1.14.101** · access-relay **1.3.2** · suite 528/528). Closed last 3 batches = 35; OPEN now = 16. Just shipped (owner-verify on a real export): CV admin template rebuild (1.50.995) + contact-line sidebar bridge no-shrink (worker 1.14.101) + generic CL template 991→994. _(prior snapshot:)_ **`docs/qa/PROJECT_ISSUES_OPEN_CLOSED_2026-06-29.md`** + per-item plans in `docs/qa/NEXT_SESSION_2026-06-29.md` (PWA **1.50.968** + docx-worker **1.14.96** + access-relay **1.3.2**; suite 521/521). Read those FIRST. CRITICAL data-loss FIXED (relay 1.3.2 + client guard 1.50.957). Top OPEN = (1) kernel role bullets/results + Students-Council dup (owner rules: hide bullet not result; persistence), (2) candidate-header 3-col-grid placement, (3) CL sign-off/rich_block-compress/slogan-panel, (4) quick-gen page convergence. Older: `NEXT_SESSION_2026-06-28.md`.
>
