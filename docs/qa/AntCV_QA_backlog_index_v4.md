...
## Session 2026-06-11 additions (VERB-LED-001, GEN-BACKGROUND-001-CLIENT, JD-ANALYSIS-PRINT-001, CA-006, DATA-DUP-001, WM-006, PDF-LAYOUT-002)
...
- **CA-006** _(Medium)_ — Application label bleeds into the first experience role title in Preview. With an application label set ("Trackman"), the first role rendered as "Application: Founder & Product / Project Expert - Trackman, Kanzen konsulenter i nord ApS". The clean DOCX/PDF is correct ("Founder & Product / Project Expert | Kanzen konsulenter i nord ApS"), so this is **Preview-only** — the candidate-band/role-line composer concatenates the applicationLabel into the role/company string. Related to CA-002 (application sentence model) but distinct: this is the role line in PROFESSIONAL EXPERIENCE, not the header sentence. **Fix (not started):** keep applicationLabel in the header sentence only; never inject it into the experience role/company line.
- **DATA-DUP-001** _(Medium, data decision)_ — Innoviz appears as three overlapping roles: "System Architect & Change Control Lead 2020-2025", "Customer Change Requests Specialist 2020-2025", "System Architect 2017-2020". The two 2020-2025 entries share dates and overlap heavily, reading as a duplicate. This is a **source-kernel data** issue, not a render bug. **Decision needed (owner):** canonical shape is two roles — merge the two 2020-2025 entries into one "System Architect & Change Control Lead 2020-2025" (folding the CRM bullets in), keep "System Architect 2017-2020" separate. Edit lives in the kernel CV data, not code.
- **WM-006** _(Low)_ — AI-assisted notice runs long in the CV main column ("AI-assisted — author retains responsibility for content."). Cosmetic; the footer/notice sidecar can shorten or re-wrap it. Sibling to WM-001..005 (watermark family) but this is the textual AI notice, not the diagonal watermark.
