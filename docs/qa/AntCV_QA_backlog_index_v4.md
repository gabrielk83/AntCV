...
## Experience editor — kernel role seeding (EXP-HIDDEN-ROLES-001)

- **EXP-HIDDEN-ROLES-001** _(High)_ — Kernel work-history roles render as blank "New role" slots in the experience editor instead of populated, hidden entries. Reported on a tailored application (Project Manager, R&D Group @ NKT Photonics): the editor showed three empty "New role" input rows at the bottom while TAU, IDF, and Meprolight (present in the candidate kernel) were absent — forcing the user to retype role content from memory. Root cause (`pwa/app.src.js`, experience section-merge): on the tailored path (`p`/`g` false), the merge built the role list from the editor list `r` overlaid with LLM `experience_roles`, then a merge-in loop pushed only LLM roles missing from `r` — it never pushed kernel work-history roles missing from both. A kernel role absent from `r` and from the LLM output was dropped, and the editor padded with empty templates. The data model already supports populated-but-hidden roles (`on:false`; `kernel-completeness-290` skips `r.on===false`), so the gap was purely seeding. Fix (two parts): (A) editor backfill — after the existing merge-in loop, push any stored work-history role missing from the final list as `{...kernelContent, on:false}`, matched by id or by title+company to avoid duplicating a role already present under a different id; bullets carried from kernel, never blank. (B) generation prompt — the experience-roles instruction now mirrors the sidebar "INCLUDE ALL, MARK IRRELEVANT ONES on:false" rule, so the LLM returns every kernel role (irrelevant ones `on:false`, fully populated) rather than dropping them. _Shipped (committed to main; bundle redeployed after a placeholder-corruption incident on the same commit was repaired)._ **Acceptance:** open a tailored application whose JD makes early-career roles irrelevant; every kernel role must appear in the experience editor — visible ones `on`, the rest populated and toggled off — with zero empty "New role" slots, and toggling a hidden role on must restore its real title/company/years/bullets without retyping. Verify in Preview, DOCX, and PDF.
