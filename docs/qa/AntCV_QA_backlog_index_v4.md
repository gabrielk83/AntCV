...
- **SO-004** _(High, crash, open)_ — React error #185 (maximum update depth exceeded) while typing in the Selected Outcomes editor. App 1.50.386, Android Chrome 149, 2026-06-12 07:27:37Z, during the manual SO-003 recovery paste. Breadcrumbs: sustained `input`/`change` events on `input[Outcome text]` (and `input[[Verb]]`); the crash fires on a `change` commit, stack passes through `app.js?v=1.50.383-cjlr-clean` into react-dom 18 production. Suspect: a setState-in-effect loop in the SO row component — an effect that writes back to the same state feeding the input's value prop (likely in the per-row controls or SO-002 add-behavior code), entering an infinite update cycle once a change commits. **Secondary observation:** the page reports app 1.50.386 but loads `app.js?v=1.50.383-cjlr-clean` — stale cache-bust param; confirm the bundle param matches the release before deeper triage (the crash may already be fixed in the real .386 bundle, or the param is simply not bumped). **Triage:** reproduce in a non-minified build; inspect SO row effects for value-prop-triggered state writes; check whether the crash drops the in-flight edit (data-loss adjacency with SO-003).
...
## Session 2026-06-12 additions (GEN-SCE-FLAG-001, GEN-LANGFAB-001, SO-004)
