...
| DEMO-PERSIST-001 | `OPEN` | `AntcvSetUserMode("demo")` + reload still returns relay `user_mode:"paid"` — demo mode won't activate (diag-confirmed). | `[console]` SET-MODE POST status, then `[worker]` if write fails |
