...
| **DATA-EXPORT-001** | Personal menu: download stored data + analytics to a protected file. | APP.JS | CORE SHIPPED | **Owner-decided: crypto.** Tested core `pwa/lib/data-export.js` (PBKDF2→AES-GCM, 11 unit tests) shipped. Remaining `[console]` wire the Personal-menu download + passphrase prompt to `window.AntcvDataExport.exportEncrypted`. |
