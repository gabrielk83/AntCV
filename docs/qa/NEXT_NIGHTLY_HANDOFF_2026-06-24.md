...
- **Boot-freeze remaining swarm** — `antcv-language-ui-429.js` DONE (BOOT-LANGUI-GATE-001, 1.50.870). The three then-named offenders — `antcv-core-wib-strict-row-layout-274.js` (227→112ms), `antcv-selected-outcomes-row-controls-237.js` (143→107ms), `antcv-embedded-controls-248.js` (134→108ms) — DONE (BOOT-COREWIB/OUTCOMES/EMBED-PERF-001, 1.50.880): per-run `clean` memo + 274 visible-after-text reorder. native/gc 2876→2098ms. NEXT profiled offenders: `antcv-profile-workstyle-cjlr-238.js` (sectionFromElement 111ms residual after its earlier lowText memo), `antcv-what-i-bring-header-cjlr-249.js` (editorRoot 82ms), `antcv-how-contribute-controls-245.js`. Bigger lever = a SHARED swarm coalescer — higher value, higher blast radius. Re-profile with `diag-boot-profile.mjs` after each.
