# AntCV nightly — 2026-08-13 (CI / GitHub Actions, unattended, Opus 4.8)
...
already up to date, base HEAD `2ee32ca1` (the 08-12 CI-nightly report commit). Working tree clean
...
A quiet night: **nothing has landed since the 08-12 CI report** — HEAD is that report commit itself.
...
## Code delta since the 08-12 CI report (`2ee32ca1` → HEAD)
**Empty.** HEAD == `2ee32ca1` (the 08-12 CI report). `git diff --stat 2ee32ca1..HEAD -- pwa/ workers/
.github/` = **empty**. No commit of any kind has landed since last night. No PR owed.

## Standing probes — ALL GREEN on main (`2ee32ca1`, PWA live `1.51.4086-demand-seed-refresh`)
- **PWA suite:** `node scripts/run-tests.mjs pwa` → **1570/1570 pass**, 0 fail.
- **Full-repo suite:** `node scripts/run-tests.mjs` → **1893/1893 pass**, 0 fail. Flat vs 08-12.
...
- **copenhagen-overflow-storm** — PASS. First cold run read ON 3 writes / 15px usablePx-drift / 0 err;
  two immediate re-runs converged to the baseline ON 2 writes / 0px / 0 err (the 15px is the
  documented one-time mount-settle transient, not a storm — write-count stayed bounded, DIAG PASS
  on every run). OFF (`antcv:copenhagen-v2=0`) 1 write / 0px / 0 errors. Preview converges both ways.
- **settings-panels-probe** — DIAG PASS. Layout 0 mutations/6s, `rootFound=true`, 0 page errors.
- **panel-button-audit** — 209 buttons, **0 page errors**, 133 active, 14 skipped-dangerous,
  17 ui-only, 45 not-visible-or-disabled. Artifacts `docs/qa/PANEL_BUTTON_AUDIT_2026-08-13.{json,md}`.
- **sidebar-stable** — OK. width/height stable + converged after 12 scrolls (w0==w1==262.02,
  heightConverged), 0 style writes, 0 errors.
...
Code surface byte-identical for pagination/render since 08-12 (no commit at all landed); chromium
availability let me genuinely re-verify the stalest render-gated rows instead of carrying them:
- **Row 1** (quick-gen / 3-page convergence) — verified: 2026-08-13; all render-gated diags green today.
- **Row 3** (floating spine) — verified: 2026-08-13; flag default-OFF re-confirmed against current
  source (docx-worker:24674 `floatSpine=payload.float_spine===true||style.floatSpine===true`; docx-client
  gated on `localStorage antcv:float-spine==='1'` at :1251/1253); gate logic intact, no code delta.
- **Row 11** (sidebar page-2/3 dance, CLOSED) — re-verified: 2026-08-13; sidebar-stable +
  sidebar-promote-margin both OK.
- **Row 17** (settings sweep-army, DONE 1.51.156) — re-verified: 2026-08-13; settings-panels-probe
...
- **LLM-TRAFFIC-GAP-2026-08 / RELAY-TUNE-COVERAGE-GAP-001** — carried. No LLM call of any task since
  2026-07-30 (now a **14+ day gap**); no fresh-gen content check is possible and the weekly tune's
  next run is ~08-13 (its dedicated routine, not this nightly).
