...
### Salmon follow-ups (owner 2026-06-22, after 1.50.749/750 force-break) — `[SALMON-EMPTY-REGION-001 SHIPPED 1.50.753; SALMON-PAGE3-MISSING-001 SHIPPED 1.50.751]`
...
| Item | Why |
|---|---|
| ~~P0 SALMON-EMPTY-REGION-001~~ | **SHIPPED 1.50.753** (see CLOSED block below). The circular lock had a THIRD actor the cascade map missed: `antcv-sidebar-subsection-pagebreaks-329.js` injects `.antcv-page-row,.antcv-document-sidebar{min-height:1123px!important}`. Coordinated fix in `antcv-page-fit.js` (non-last row `min-height:0!important`) + `antcv-sidebar-fill-equalize-227.js` (non-last row sizes the navy to the TALLER column's CONTENT height via children-sum, inline `!important` beating the stylesheet). Last row keeps A4. Verified `diag-salmon-empty-region.mjs`: row0 1123→957 (content 931), salmon gap 192→26px, stable across 6 cycles (no oscillation), last row 1123, suite 366/366, boot-smoke clean, export untouched. |
...
## UPDATE — 2026-06-22 cloud routine #3 (1.50.753)
