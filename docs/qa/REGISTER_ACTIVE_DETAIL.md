...
```
| **102** | **DEMAND-SEED-SEARCH-TOKEN-MISSING-001 (found by the weekly demand-seed run 2026-08-26, first run to PROBE rather than assume).** the routine's prescribed search backend is unreachable from an unattended run. `POST /api/research` gates on an owner session JWT (`identityFromRequest`) → 401 `unauthenticated` headless. `GET /api/cse-search` gates on the machine token `CSE_PROXY_TOKEN` (`x-antcv-cse-token` header) → 401 `unauthorized`, because that token is NOT provisioned on this machine (only `CLUSTER_RESEARCH_TOKEN` is, which is the WRITE token and does not open the search leg). So the Brave-backed **site-scoped** search (`siteSearch=jobindex.dk`, Glassdoor) has been silently unavailable to every run of this routine since the Brave switch, and Danish evidence keeps arriving second-hand via plain WebSearch (this run: the IT-Branchen/Jobindex analysis) instead of from Jobindex postings directly. NOT a code bug — the headless-friendly route already exists and works by design. **OWNER-OWED one-line fix:** set `CSE_PROXY_TOKEN` as a Windows User env var on the desktop, same value as the relay secret, exactly as `CLUSTER_RESEARCH_TOKEN` already is; then the routine regains Nordic site-scoped coverage with no code change. Note this supersedes the optimistic framing in `docs/deployment/google-cse-setup.md` §6 ("UNBLOCKED 2026-08-18 via Brave") — the backend is unblocked, the routine's access to it is not.  **ESCALATED 2026-09-14 (weekly demand-seed run, cloud routine):** `CSE_PROXY_TOKEN` is now provisioned (39 chars, well-formed) but the relay STILL refuses it, and the WRITE token has regressed with it — `GET /api/cse-search` and `POST /api/cluster-demand-research` both 401 `{"error":"unauthorized"}`, though the same write route accepted its token on 2026-08-26 (`total_inserted:226`). Relay is UP and the handlers ARE reached (an unknown path returns the different `unauthenticated` body), so this is the `tok !== env.<TOKEN>` comparison failing, not an outage or a code bug; both env values are clean (no placeholders/quotes/whitespace, re-probed trimmed). Diagnosis: Worker-secret vs routine-env DRIFT — the working 2026-08-26 push ran from the desktop, this run is the cloud routine. Fix (owner-owed, no code): `wrangler secret put CSE_PROXY_TOKEN` + `CLUSTER_RESEARCH_TOKEN` on `antcv-access-relay`, or correct the routine's env vars, then re-run. **Consequence: the 2026-09-14 weekly run produced NOTHING** — no research pass, no seed change, no D1 write (spec says STOP on a 401). D1 verified clean and consistent: all 9 clusters still 20 rows, ranks 1..20, nothing mid-edit. The hand-rolled-SQL fallback was deliberately NOT used — it is authorised only when the relay is DOWN.  **ROOT-CAUSED 2026-09-21 (second consecutive blocked week) — the 2026-09-14 "drift" diagnosis was wrong; the env value is the WRONG KIND of secret.** Both routes re-probed, identical 401s (the write leg probed with an empty `{"clusters":{}}` body, which would 400 `no_known_clusters` if the token were accepted — isolates auth, writes nothing; unknown-path control still returns the different `unauthenticated`, so the relay is UP and both handlers are reached). Length is not the test, KIND is: `docs/deployment/google-cse-setup.md` §2 defines a 39-char `AIza`-prefixed string as **`GOOGLE_CSE_KEY`** (the billable Google credential) and §4 defines `CSE_PROXY_TOKEN` as **`openssl rand -hex 32`** (64 hex chars, no prefix). The routine's `CSE_PROXY_TOKEN` env var is **39 chars beginning `AIza`** — it is a Google API key in the proxy-token slot, so no rotation of the relay secret can ever make it match. **Credential-hygiene consequence (owner action):** §4 explicitly warns never to paste `GOOGLE_CSE_KEY` into "a commit, PR body, screenshot, or a scheduled-trigger prompt", and a value of exactly that shape is in this scheduled routine's env — **rotate the Google key**. It also buys nothing: the endpoint's backend is Brave (`env.BRAVE_API_KEY`) since `CSE-PROXY-GOOGLE-ENTITLEMENT-001`, so `GOOGLE_CSE_KEY` is not even the live upstream key. The value was never printed, logged or committed (shape read, value never emitted). **Fix, ordered:** (1) rotate the `AIza…` key in Google Cloud; (2) set `CSE_PROXY_TOKEN` in the routine env to the relay's self-issued random token, not an `AIza…` key, and re-sync `CLUSTER_RESEARCH_TOKEN` (51 chars, also refused); (3) if the relay-side values are unknown, `wrangler secret put` both on `antcv-access-relay` and paste the same two into the routine config; (4) verify in 30s — `/api/cse-search?q=test` should return `{"ok":true,"source":"brave"}` and `POST /api/cluster-demand-research` with `{"clusters":{}}` should return **400**, not 401. **Consequence: the 2026-09-21 run also produced nothing** — seed and `__global_market__` rollup both last refreshed 2026-08-26, now 26 days stale. No D1 writes attempted, so the rollup is untouched; this run could not re-run the verifying `SELECT` itself (the Cloudflare D1 MCP server needs interactive OAuth a routine cannot perform, and no token-gated read route exists) — filed as a follow-up: a read-only `GET /api/cluster-demand-research` under the same token would let every future run self-verify. | OPEN — filed 2026-08-26, verified: 2026-09-21 |
```
