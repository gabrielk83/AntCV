...
(function(){
  'use strict';
  const VERSION='1.50.209-hiwc-btn-wrap';
  ...
  function rowHostForField(f,key){if(!f||isInPreviewPaper(f))return null;const p=f.parentElement;if(!p)return null;p.style.display='flex';p.style.alignItems='center';p.style.gap='4px';p.style.flexWrap='wrap';let h=p.querySelector(':scope > [data-antcv331-host="'+key+'"]');if(!h){h=document.createElement('span');h.setAttribute('data-antcv331-host',key);p.appendChild(h);}h.innerHTML='';return h;}
