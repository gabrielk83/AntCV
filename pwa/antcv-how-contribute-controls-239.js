/* AntCV How I Would Contribute controls (v1.40.243)
...
(function(){
  ...
  function injectCss(){
    if(document.getElementById('antcv-hiwc-243-css'))return;
    const st=document.createElement('style');st.id='antcv-hiwc-243-css';st.textContent=`
  ...
  function syncSectionField(k,v){
    ...
    if(k==='bullets'){s.bullets=v.split(/\n+/).map(clean).filter(Boolean);}
    writeJson(SECTIONS_KEY,all);pulse();
  }
  ...
  function renderBulletList(r,ta){
    ...
    const box=document.createElement('div');box.setAttribute('data-antcv-hiwc-bullet-list','1');Object.assign(box.style,{display:'flex',flexDirection:'column',gap:'4px',margin:'4px 0',width:'100%'});
    ...
    function draw(){
      box.innerHTML=''; const current=bulletRowsFromText(getVal(ta)); if(!current.length) current.push('');
      current.forEach((txt,idx)=>{
        const key='bullet_'+idx;
        const row=document.createElement('div');row.setAttribute('data-antcv-hiwc-bullet-row','1');Object.assign(row.style,{display:'flex',alignItems:'center',gap:'3px',width:'100%'});
        ...
        del.onclick=ev=>{ev.preventDefault();ev.stopPropagation();const vals=currentInputs().map(x=>x.value.trim());vals.splice(idx,1);syncBulletTextarea(ta,vals.filter(Boolean));draw();applyPreview();};
        row.appendChild(inp);row.appendChild(page);row.appendChild(comp);row.appendChild(enr);row.appendChild(cjlr);row.appendChild(del);box.appendChild(row);
      });
      const add=document.createElement('button');add.type='button';add.textContent='+ Bullet';Object.assign(add.style,{alignSelf:'flex-start',border:'1px solid #008b8b',background:'white',color:'#006b6b',borderRadius:'4px',padding:'2px 8px',cursor:'pointer'});
      add.onclick=ev=>{ev.preventDefault();ev.stopPropagation();const vals=bulletRowsFromText(getVal(ta));vals.push('');syncBulletTextarea(ta,vals);draw();};
      box.appendChild(add);
    }
    ta.style.display='none';ta.parentNode&&ta.parentNode.insertBefore(box,ta);draw();
  }
