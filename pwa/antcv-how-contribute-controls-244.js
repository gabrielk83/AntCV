...
(function(){
  ...
  function injectCss(){
    if(document.getElementById('antcv-hiwc-244-css'))return;
    const st=document.createElement('style');st.id='antcv-hiwc-244-css';st.textContent=`
  ...
  function syncSectionField(k,v){
    ...
    if(k==='bullets'){s.bullets=v.split(/\n+/).map(clean).filter(Boolean);s.items=s.bullets;}
  ...
  function syncPreviewBulletNodes(secEl,p){
    ...
    if(!list){list=document.createElement('ul'); if(p.closing&&p.closing.parentNode)p.closing.parentNode.insertBefore(list,p.closing); else secEl.appendChild(list);}
    const lis=Array.from(list.children).filter(x=>/^(LI)$/i.test(x.tagName));
