/* AntCV stability core (v1.40.333)
...
(function(){
  ...
  function writeLangs(arr){
    ...
    try{var p=readJSON('antcv:prefs')||{}; p.enabledLanguages=next; localStorage.setItem('antcv:prefs',JSON.stringify(p));}catch(_){}
  ...
  function makeLanguageCard(){
    ...
    card.style.cssText='margin-top:16px;border:1px solid rgba(255,255,255,.16);border-radius:8px;padding:12px 14px;background:rgba(255,255,255,.03);color:#d7e6ee;';
    card.innerHTML='<div style="font-weight:800;letter-spacing:.02em;margin-bottom:8px;color:#d7e6ee;text-transform:uppercase">Languages in the top bar</div><div style="font-size:12px;opacity:.78;margin-bottom:10px">Choose which language buttons are available in the top bar. This does not start translation.</div><div data-lang-options style="display:grid;grid-template-columns:repeat(2,minmax(120px,1fr));gap:8px"></div>';
    return card;
  }
  ...
  function forceRoute(){
    ...
    if(!route || Date.now()-Number(route.ts||0)>10000) return;
