...
(function(){
  ...
  function panels(){const seeds=Array.from(document.querySelectorAll('input,textarea,[contenteditable="true"]')).filter(f=>/focus area/i.test((f.value||f.placeholder||f.textContent||'')));const roots=[];seeds.forEach(f=>{let p=f.parentElement,best=null;for(let d=0;p&&p!==document.body&&d<10;d++,p=p.parentElement){const t=clean(p.textContent);if(/\+\s*Row/i.test(t)&&/strategic expertise/i.test(t))best=p;if(targetFromText(t)){best=p;break;}}if(best&&!roots.includes(best))roots.push(best);});return roots.filter(visible);}
  function rows(root){const seeds=Array.from(root.querySelectorAll('input,textarea,[contenteditable="true"]')).filter(f=>/focus area/i.test((f.value||f.placeholder||f.textContent||'')));const out=[];seeds.forEach(f=>{const r=rowForField(f,root);if(r&&visible(r)&&!out.includes(r))out.push(r);});return out;}
  ...
  function normalize(){panels().forEach(root=>{const target=targetForPanel(root);if(!target)return;const sid=targetSid(target);rows(root).forEach((row,idx)=>{if(idx===0){row.querySelectorAll('[data-antcv-table-page-328]').forEach(x=>x.remove());return;}let b=row.querySelector('[data-antcv-table-page-328]');if(!b){b=makeBtn();hostFor(row).appendChild(b);}paint(b,sid,idx);b.onclick=function(ev){ev.preventDefault();ev.stopPropagation();if(ev.stopImmediatePropagation)ev.stopImmediatePropagation();setPage(sid,idx,getPage(sid,idx)%4+1);paint(b,sid,idx);};});});}
