...
if (anchored) {
  ...
  await page.evaluate(({port})=>{
    ...
    inputs[0].value='Mock-70B'; inputs[1].value='http://127.0.0.1:'+port+'/v1'; inputs[2].value='mock-70b';
    inputs[3].value='k'; inputs[4].value='0.5'; inputs[5].value='1.5';
    [...host.querySelectorAll('button')].find(b=>/Save \(pending audit\)/.test(b.textContent)).click();
  },{port});
  await page.waitForTimeout(400);
  let store = await page.evaluate(()=>JSON.parse(localStorage.getItem('antcv:customLlms')||'[]'));
  check('2. added with status pending', store.length===1 && store[0].status==='pending' && store[0].model==='mock-70b', JSON.stringify(store));

  // 3 — Approve locked pre-audit
  ...
  check('3. Approve locked before audit', preApprove.present && preApprove.disabled, JSON.stringify(preApprove));

  // 4 — run the audit (mock endpoint passes all critical probes)
  await page.evaluate(()=>{ [...document.getElementById('antcv-llm-lab').querySelectorAll('button')].find(b=>/Run audit/.test(b.textContent)).click(); });
  await page.waitForTimeout(2500);
  store = await page.evaluate(()=>JSON.parse(localStorage.getItem('antcv:customLlms')||'[]'));
  const audit = store[0] && store[0].audit;
  check('4. audit battery passes (instruction+JSON+banned)', !!(audit && audit.pass && audit.probes.instruction.pass && audit.probes.json.pass && audit.probes.banned.pass), JSON.stringify(audit));
