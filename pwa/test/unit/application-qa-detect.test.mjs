...
test('end-to-end with P1: after the bridge writes, the scaffold builds application_qa after closure', async () => {
  ...
  // QA-HEADER-TRIM-001 (1.51.111): no candidate-header row — the section
  // TITLE is the page headline; items are Q&A rows + the closing line.
  assert.equal(qa.items.length, 3); // 2 Q&A rows + closing line
  assert.equal(qa.title, 'Responses to application questions:');
  assert.equal(qa.items[0].b, TWO_QS[0].question);
});
