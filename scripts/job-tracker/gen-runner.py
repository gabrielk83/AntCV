...
CL_SECTIONS = [
    ...
    ("cl_slogan",           "SLOGAN",            "Write ONE cover-letter HEADLINE (the slogan), a single line of 4 to 13 WORDS (V5-SLOGAN-ROLE-001, v5 §1 'outcome-oriented headline LINKED TO THE ROLE'). SHORTER, SHARPER AND MORE MEMORABLE SCORES HIGHER - 13 is a hard ceiling, NOT a target; use the FEWEST words that still land the point. PREFERRED SHAPE: 'A [candidate's real role identity] WHO [concrete, role-specific value verb + domain object]' - the GOLD STANDARD is 'A PROJECT MANAGER WHO MOVES OPTICAL HARDWARE FROM LAB TO SCALABLE DELIVERY'; a vague tagline that could head ANY letter is a FAILURE. ORGANISING PRINCIPLE = THE ROLE + THE OUTCOME: name what this ROLE does and the concrete OUTCOME the candidate moves it toward - e.g. 'Moving optical hardware from lab to scalable delivery' (8) or 'Project manager turning lab optics into product' (7). It must read as THIS role, not as a generic personal tagline. SECONDARY (never at the cost of the role linkage or of brevity): if an EMPLOYER BRAND block is present you MAY echo its register/tone, but NEVER name the company and NEVER invent a company value. Hard limits: 4-13 words, no trailing period, no quotation marks. Return ONLY the line."),
]
