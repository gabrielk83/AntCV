...
async function handleRequest(request, env = {}) {
  ...
  try {
    ...
    // The PWA / prompt-augment can each produce a body with
    // messages[0].role === 'system'; we normalise here so whatever
    // upstream shape arrives, Anthropic gets a valid request: lift
    // every leading system message into the top-level `system` string
    // (merging with any existing top-level system).
