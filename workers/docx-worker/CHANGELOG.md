...
- APPLINE-EDIT-001 (owner 2026-07-29 "allow editing of the slogan and of the application line in the preview"): the cover-letter APPLICATION LINE is editable inline in the preview now, over the standalone `antcv:clAppLine` override, and the client forwards it as `meta.app_line`. The worker prefers that override and SHORT-CIRCUITS its own composition instead of being overwritten by it — the first cut cleared `__role` but left `__company`, so the `if (__role || __company)` branch recomposed "Application for … at <company>" straight over the owner's text. Now the whole composition block is gated on `!__alText`. An absent `meta.app_line` leaves every existing document byte-identical (PWA 1.51.3982-inline-edit-appline).

## 1.14.173-hdr-type-ctrl

- HDR-TYPE-CONTROLS-001 (owner 2026-07-29): the Font sizes (pt) panel now drives SIZE **and LETTER SPACING** for the five identity lines on both documents, and the worker honours every value. New `font_sizes` keys: `applicationSize`, `sloganSize` (sizes, pt) and `nameTrack`, `specTrack`, `applicationTrack`, `contactTrack`, `sloganTrack` (letter-spacing DELTAS, pt, 0.05 steps). 0.05pt is exactly one twentieth of a point = the DOCX `w:spacing` unit, so a panel step maps onto Word character spacing losslessly. What changed: the CL slogan run was hard-pinned at 11pt / `characterSpacing` 20 and the CL application line at 10.5pt / 4 — both now read the payload; the header name / specialisation / contact runs add the track delta on top of whatever tracking that package already used (copenhagen .14em name fit, spec 11, contact -2 with `w:w=73`, bridge -10, legacy none). `__cphNameFit` adds the name delta to its fitted track so the measured copenhagen fit cannot outrank an explicit owner choice. DELTA semantics: a payload with no track keys, or all-zero tracks, is byte-identical to 1.14.172. Test: `diag-hdr-type-controls.mjs` (12 checks incl. the zero-delta inertness proof).

## 1.14.160-edu-row-cjlr-export

- EDU-ROW-CJLR-EXPORT-001 (owner 2026-07-15): per-row EDUCATION / RECOMMENDATIONS CJLR now EXPORTS, matching the preview shipped in PWA 1.51.1424-edu-row-cjlr. The preview + education editor write antcvItemAlignment[sid]["items.N.deg"] (+ "items.N"), N = the ORIGINAL s.items index; renderEducation now resolves each row's paragraph alignment via paraAlignPath(s, "items.N.deg") ?? paraAlignPath(s, "items.N") (recovering the original index with s.items.indexOf(it), since the visible/dedup filter drops it). NO __group__ fallback: the preview's education render (__eRA) ignores __group__ and item-align never applies it to non-group-head rows, so __group__ must not move education rows on export either (consistent with GROUP-CJLR-SCOPE-001, 1.14.158). Default = docx LEFT = preview default. Byte-identical for any doc with no per-row education override AND no __group__ (the removed groupCjlr term was undefined then). The CONT. continuation heading is unchanged (still honours groupCjlr as a heading). Test: diag-cjlr-edu-export.mjs asserts a per-row override moves that education row and leaves the others at left, and that __group__ does NOT move education rows.

## 1.14.159-bullet-left-parity

- PREVIEW-BULLET-PARITY-001 (owner 2026-07-15): experience role BULLETS now default to LEFT in the export (was JUSTIFIED), matching the preview. Experience is a grouped rich_block (each role line is a group head), so the preview resolves role bullets via __rowAlign(isGrp=false, __hasGrp=true) -> "left" (verified live on antcv.pages.dev: roles.N.bullets.M compute textAlign:left). renderExperience bAlign now falls back to AlignmentType.LEFT instead of undefined (which bulletParagraphRich turned into JUSTIFIED). A per-bullet CJLR override still wins; __group__ still never touches bullets (GROUP-CJLR-SCOPE-001). Role LINE default stays space-between in BOTH export and preview (the preview's center-by-default role heading is a preview bug being fixed separately in the PWA, per owner). Test: diag-cjlr-group-role-export.mjs asserts role bullet default = left.

## 1.14.158-group-cjlr-scope

- GROUP-CJLR-SCOPE-001 (owner 2026-07-15): the __group__ ("Groups" / align-all-groups) control now aligns GROUP HEADS ONLY in the export, not content rows — closing the residual parity gap noted at 1.14.156. The preview __rowAlign (app.src.js) resolves content-row align as `isGrp ? __al.__group__ : void 0` so __group__ never touches body rows; the worker's renderRichBlock content-row align and renderExperience bullet align still fell back to __group__ (groupCjlr / paraAlign(s,null)), so setting the Groups control to e.g. right also right-aligned every rich_block content row + role bullet in the export while the preview kept them at their default. Removed the __group__ fallback from both content paths (group HEADS + role LINES still honour __group__ via their own galign/roleAlign). Byte-identical when __group__ is absent (the removed term was undefined then); when set, content rows now stay at their default (grouped rich_block -> left, non-grouped -> justify, role bullets -> justify) exactly as the preview. Test: diag-cjlr-group-role-export.mjs adds a scope block (group head + role line move; content row + role bullet do NOT).
...
## 1.14.155-slogan-brand-color

- SLOGAN-BRAND-COLOR-001 (owner 2026-07-14): the cover-letter SLOGAN run now follows the BRAND slogan colour instead of the hardcoded package teal, matching the preview (which colours the slogan via `var(--brand-slogan-color, <teal>)` set on the paper wrapper from the fitted brand, PWA 1.51.729). The client (antcv-docx-client.js, PWA 1.51.1304-slogan-brand-color) forwards `meta.slogan_color` from `antcv:brandV2` slots.sloganColor — the SAME source the preview reads — gated by the same `window.__antcvBrandFit` flag; it is OMITTED when no brand is active so the worker keeps its teal. The worker's CL slogan TextRun now renders `meta.slogan_color ? sloganColorOnWhite(meta.slogan_color, style.mainHeadColor) : style.mainHeadColor`. ACCESSIBILITY / CONTRAST-GUARD (STANDING owner rule, TABLE-HEADER-INK-001 pattern): the slogan is coloured text on the WHITE page, so a new `sloganColorOnWhite(hex, fallback)` helper (near readableInk) DARKENS the colour (hue kept) until it clears ~3:1 luminance contrast against white — a colour token never ships unreadable. Defence-in-depth: the client applies the identical guard before forwarding, the worker re-guards on render. Byte-identical for any doc with no `meta.slogan_color` (the field is inert-when-absent). Test: diag-cl-slogan-brand-color.mjs (brand colour renders as-is / absent -> package default teal / too-light `FFDD00` darkened `AB9400` to 3.01:1).
