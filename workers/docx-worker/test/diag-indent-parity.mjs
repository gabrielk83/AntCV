...
// 1.14.49 bullet-marker offset: marker sits at left − hanging = 45 DXA (3px),
// text at bulletIndent×15. Default 14px → 210/165; custom 24px → 360/315.
const defOk = def.inds.includes('210/165') && def.tcLefts.includes(150);
const cusOk = cus.inds.includes('360/315') && cus.tcLefts.includes(300) && !cus.tcLefts.includes(150);
const markerAt45 = [...def.inds, ...cus.inds]
  .filter((x) => x === '210/165' || x === '360/315')
  .every((x) => { const [l, h] = x.split('/').map(Number); return l - h === 45; });
log('default 210/165 + edge 150:', defOk, '| custom 360/315 + edge 300:', cusOk, '| marker at 45 DXA:', markerAt45);
const ok = defOk && cusOk && markerAt45;
